...
class ProxyRotator():
    ...
    def __get_proxies(self, under_min=False) -> None:   # done
        try:
            if under_min:
                self.__logger.info("Fetch new proxy lists from internet.")
            ...
            self.__logger.warning("{0} does not exists! Fetch proxy lists from internet.")
    ...
    def rotating_requests(self, url, timeout=None, proxy=True) -> requests.Response:
        index = self.__request_counter % len(self.__valid_proxies)
        
        if not timeout:
            timeout = self.__start_timeout
        proxies = {"http": self.__valid_proxies[index], "https": self.__valid_proxies[index]} if proxy else None

        self.__logger.info("proxy {0:25s} timeout={2:2d} request {1}".format(self.__valid_proxies[index], url, timeout))
        self.__request_counter += 1
        try:
            ...
            if timeout < self.__max_timeout:
                return self.rotating_requests(url, timeout*2, proxy)   # runs out without to do something
        except ProxyError:
